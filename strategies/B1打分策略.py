...
调用 signals/singal_cal/B1_strategy_module 的 calculate_b1_score，
通过 basic_module.calculate_indicators 生成指标字典。
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'signals', 'singal_cal'))
...
@register(name='B1打分策略', threshold_required=True, min_data_days=120, description='天宫B1完整打分策略')
class TemplateStrategy(BaseStrategy):
    ...
    def _calculate_buy_score(self) -> float:
        ...
        import pandas as pd
        n = len(self)
        close_arr = np.array(self.close.array[:n])
        open_arr = np.array(self.open.array[:n])
        high_arr = np.array(self.high.array[:n])
        low_arr = np.array(self.low.array[:n])
        volume_arr = np.array(self.volume.array[:n])
        ...
        try:
            ...
            return calculate_b1_score(indicators)
        except Exception:
            return 0.0
